...
// ── .md.ini — server-side hard overrides ────────────────────────────────────
// This file lives next to md.php and is never served as a document.
// Constants defined here cannot be changed via cookies or the Settings panel.
// To change behaviour, edit .md.ini directly on the server.
//
// Supported keys (all optional):
//   DISABLE_UPLOAD    = true|false   (default: true)  — hides & blocks .md upload
//   DISABLE_CLIPBOARD = true|false   (default: false)  — hides & blocks clipboard preview
...
INI;
    ...
    $featureKeys = [
        'AUTO_NUMBERING', 'AUTO_TOC', 'AUTO_FOOTNOTES_LINKS', 'DOUBLE_LINE_BREAKS',
    ...
    foreach ($featureKeys as $k) {
        if (array_key_exists($k, $ini)) {
            // Re-define: PHP doesn't allow redefining constants, so we store
            // overrides in a global map; feat() will check it first.
            $GLOBALS['_MDV_INI_OVERRIDE'][$k] = $ini[$k];
        }
    }
...
{
    // .md.ini hard overrides take priority over cookies and defaults
    if (isset($GLOBALS['_MDV_INI_OVERRIDE'][$name])) {
        return $GLOBALS['_MDV_INI_OVERRIDE'][$name];
    }
    $key = 'mdv_feat_' . $name;
...
// NOTE: To hard-lock any setting, define it in .md.ini next to md.php.
//       Values in .md.ini override both cookies and defaults and cannot be
//       changed from the browser. See the .md.ini loader block above.
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['md_preview'])) {
    if (DISABLE_CLIPBOARD) {
        ...
        exit('Clipboard preview is disabled by server configuration.');
    }
    $raw = $_POST['content'] ?? '';
